...
def validate_similarity_dataframe(similarity_df, path):
    if similarity_df.shape[0] != similarity_df.shape[1]:
        raise ValueError(
            f"{path} is not square: shape={similarity_df.shape}"
        )

    if similarity_df.index.has_duplicates:
        raise ValueError(
            f"{path} contains duplicate row labels"
        )

    if similarity_df.columns.has_duplicates:
        raise ValueError(
            f"{path} contains duplicate column labels"
        )
    ...
    if set(row_concepts) != set(column_concepts):
        raise ValueError(
            f"{path} does not contain the same concepts in its rows and columns"
        )
    ...
    if not np.issubdtype(values.dtype, np.number):
        raise ValueError(
            f"{path} contains non-numeric similarity values"
        )

    if not np.isfinite(values).all():
        raise ValueError(
            f"{path} contains non-finite similarity values"
        )

    return similarity_df

def select_shared_concepts(
    similarity_df_1,
    similarity_df_2,
):
    ...
    if len(shared_concepts) == 0:
        raise ValueError(
            "No shared concepts were found between the two similarity matrices"
        )

    similarity_df_1 = similarity_df_1.loc[
        shared_concepts,
        shared_concepts,
    ]

    similarity_df_2 = similarity_df_2.loc[
        shared_concepts,
        shared_concepts,
    ]
...
def compute_topk_indices(
    ...
    if similarity.shape[0] < number_of_neighbours + 1:
        raise ValueError(
            f"Requested {number_of_neighbours} neighbours, but only {similarity.shape[0]} shared concepts are available"
        )
    ...
    partitioned_scores = np.take_along_axis(
        similarity,
        partitioned_indices,
        axis=1,
    )
...
def compute_mean_alignment_score(
    ...
    if neighbours_1.shape != neighbours_2.shape:
        raise ValueError(
            "The two nearest-neighbour arrays have different shapes"
        )

    common_neighbours = np.zeros(
        neighbours_1.shape[0],
        dtype=np.int64,
    )

    for concept_i in range(neighbours_1.shape[0]):
        common_neighbours[concept_i] = np.intersect1d(
            neighbours_1[concept_i],
            neighbours_2[concept_i],
            assume_unique=False,
        ).size
...
def relabel_nearest_neighbours(
    ...
    return inverse_permutation[
        observed_neighbours[permutation]
    ]
...
def read_observed_alignment_score(path):
    ...
    if "alignment_score" not in observed_df.columns:
        raise ValueError(
            f"{path} does not contain an 'alignment_score' column"
        )

    if observed_df.empty:
        raise ValueError(
            f"{path} does not contain any rows"
        )

    alignment_scores = observed_df[
        "alignment_score"
    ].to_numpy(dtype=float)

    if not np.isfinite(alignment_scores).all():
        raise ValueError(
            f"{path} contains non-finite alignment scores"
        )
...
def compute_empirical_p_value(
    ...
    for shuffle_i in range(number_of_relabellings):
        rng = np.random.default_rng(
            random_seed + shuffle_i
        )

        permutation = rng.permutation(
            neighbours_2.shape[0]
        )

        relabelled_neighbours_2 = (
    ...
    empirical_p_value = (
        ...
        number_of_relabellings + 1
    )

    return (
        empirical_p_value,
        number_at_least_as_extreme,
    )
...
def main():
    ...
    if args.number_of_relabellings <= 0:
        raise ValueError(
            "--number_of_relabellings must be a positive integer"
        )

    observed_alignment_score = (
        read_observed_alignment_score(
            args.observed_alignment_score
        )
    )

    similarity_df_1 = pd.read_parquet(
        args.llm_similarity_1,
        engine="pyarrow",
    )

    similarity_df_2 = pd.read_parquet(
        args.llm_similarity_2,
        engine="pyarrow",
    )

    similarity_df_1 = (
        validate_similarity_dataframe(
            similarity_df=similarity_df_1,
            path=args.llm_similarity_1,
        )
    )
