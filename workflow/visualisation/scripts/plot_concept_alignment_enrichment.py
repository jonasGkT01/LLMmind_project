...
def parse_alignment_score_path(path):
    ...
    if match is None:
        raise ValueError(
            f"Could not parse LLM-brain alignment-score filename: {filename}"
        )
...
def read_model_enrichments(
    ...
    if metadata["dataset"] != expected_dataset:
        raise ValueError(
            f"Unexpected dataset in {path}: {metadata['dataset']}"
        )
    if metadata["similarity_type"] != expected_similarity_type:
        raise ValueError(
            f"Unexpected similarity type in {path}: {metadata['similarity_type']}"
        )
    if (
        metadata["number_of_neighbours"]
        != expected_number_of_neighbours
        ...
        raise ValueError(
            f"Unexpected number of neighbours in {path}: {metadata['number_of_neighbours']}"
        )
    ...
    if missing_columns:
        raise ValueError(
            f"Alignment-score file {path} is missing columns: {sorted(missing_columns)}"
        )
    ...
    if duplicated_concepts:
        raise ValueError(
            f"Alignment-score file {path} contains duplicated concepts: {duplicated_concepts[:10]}"
        )
    ...
    if alignment_scores.isna().any():
        ...
        raise ValueError(
            f"Alignment-score file {path} contains invalid alignment scores for concepts: {invalid_concepts[:10]}"
        )
    if (
        (alignment_scores < 0)
        | (alignment_scores > 1)
        ...
        raise ValueError(
            f"Alignment-score file {path} contains scores outside [0, 1] for concepts: {invalid_concepts[:10]}"
        )
    number_of_concepts = len(alignment_df)
    population_size = number_of_concepts - 1
    if population_size <= 0:
        raise ValueError(
            f"At least two concepts are required in {path}"
        )
    if expected_number_of_neighbours > population_size:
        raise ValueError(
            f"{path} uses {expected_number_of_neighbours} neighbours, but only {number_of_concepts} concepts are available"
        )
    expected_alignment_score = (
        expected_number_of_neighbours/population_size
    )
    output_df = alignment_df[
        [
            "concept",
        ]
    ].copy()
    output_df["enrichment"] = (
        alignment_scores/expected_alignment_score
    )
...
def main():
    ...
    parameters_by_model = parse_model_parameters(
    ...
    for path in args.llm_brain_alignment_scores:
        ...
        if label in model_metadata:
            raise ValueError(
                f"More than one alignment-score file was provided for {label}"
            )
    ...
    if not model_dataframes:
        raise ValueError(
            "No LLM-brain alignment-score files were provided"
        )
    ...
    ax.set_title(
        "Concept-level LLM-brain alignment enrichment\n"
    ...
    ax.set_ylabel(
        "Observed alignment / hypergeometric expected alignment"
    )
    ax.grid(
        axis="y",
        alpha=0.25,
