...
def validate_similarity_dataframe(
    ...
    if similarity_df.index.has_duplicates:
        raise ValueError(f"{source} contains duplicate row labels")

    if similarity_df.columns.has_duplicates:
        raise ValueError(f"{source} contains duplicate column labels")
    ...
    if not np.issubdtype(values.dtype, np.number):
        raise ValueError(f"{source} contains non-numeric similarity values")
