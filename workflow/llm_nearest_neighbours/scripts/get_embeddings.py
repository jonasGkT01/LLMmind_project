...
def pool_hidden_state(
    ...
    if pool == "avg":
        return mean_pool_last_hidden(last_hidden_state=hidden_state, attention_mask=attention_mask,)
...
def get_safe_max_length(
    ...
    if text_config is not None:
        text_config_max = getattr(text_config, "max_position_embeddings", None,)
...
def extract_embedding_from_output(
    ...
    if torch.is_tensor(last_hidden_state):
        return pool_hidden_state(hidden_state=last_hidden_state, pool=pool, attention_mask=attention_mask,)

    hidden_states = getattr(output, "hidden_states", None,)

    if hidden_states is not None and len(hidden_states) > 0:
        return pool_hidden_state(hidden_state=hidden_states[-1], pool=pool, attention_mask=attention_mask,)
    ...
    vision_model_output = getattr(output, "vision_model_output", None,)

    if vision_model_output is not None:
        return extract_embedding_from_output(output=vision_model_output, pool=pool, attention_mask=None,)

    text_model_output = getattr(output, "text_model_output", None,)

    if text_model_output is not None:
        return extract_embedding_from_output(output=text_model_output, pool=pool, attention_mask=attention_mask,)

    raise ValueError(f"Could not extract embeddings from model output of type {type(output).__name__}")

def load_excluded_stimuli(
    excluded_stimuli_path: str,
) -> set[str]:
    with open(excluded_stimuli_path, "r", encoding="utf-8",) as input_file:
        return {
            line.strip()
...
def embed_long_text(
    ...
    chunk_token_length = max_length - n_manual_special_tokens
    ...
    for start in range(0, n_tokens, step,):
        ...
        input_ids_tensor = torch.tensor([model_input_ids], dtype=torch.long, device=device,
        )

        attention_mask = torch.ones_like(input_ids_tensor, device=device,)

        with torch.inference_mode():
            model_output = model(input_ids=input_ids_tensor, attention_mask=attention_mask, return_dict=True, use_cache=False,)

        pooled = extract_embedding_from_output(output=model_output, pool=pool, attention_mask=attention_mask,)

        chunk_embeddings.append(pooled.squeeze(0).detach().cpu().float())
...
def load_model(
    ...
    if quantization_config is not None:
        model = AutoModel.from_pretrained(model_path, quantization_config=quantization_config, device_map="auto", trust_remote_code=trust_remote_code,)
        ...
        model = AutoModel.from_pretrained(model_path, dtype="auto", trust_remote_code=trust_remote_code,).to(device)
...
def main():
    ...
    items, input_type = load_stimuli(stimuli_dir=args.stimuli_dir, excluded_stimuli=excluded_stimuli,)
    ...
    model = load_model(model_path=args.model_path, quantization_method=args.quantization_method, trust_remote_code=args.trust_remote_code,)
    ...
    if input_type == "text":
        ...
        max_length = get_safe_max_length(tokenizer=tokenizer, model=model, user_max_length=args.chunk_max_length,)
    ...
    dataframe.to_parquet(args.output, engine="pyarrow", index=True,)
